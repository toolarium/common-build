...
| `cb-container` | Container management tool for Docker and nerdctl. List images (`-l`), connect interactively (`-i`), start/stop containers, view logs, scan for vulnerabilities with trivy, and more. Supports `.cb-container` config files and distro aliases. See [`docs/cb-container.md`](docs/cb-container.md) for full documentation. |
